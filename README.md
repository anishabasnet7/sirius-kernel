...
The tasks were developed with help of AI tools for guidance, explanations, and learning purposes only.
...
In this scenario one can use the _main_ method of the **Setup** class to launch SIRIUS in the **IDE** or use
[IPL](https://github.com/scireum/docker-sirius-runtime/blob/master/src/main/java/IPL.java) via the supplied _docker_
...
or package of _customizations_. So if a customization would be named \*foo** the base package would be
...
To set up which customizations are enabled in what order specify an array for _sirius.customizations_ in the
**instance.conf**.
